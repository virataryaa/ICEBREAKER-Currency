"""
Cocoa Currency Index — ICE Connect Ingest
==========================================
Usage:
    python cocoa_ingest.py            # incremental update
    python cocoa_ingest.py --full     # full pull from 2014-01-01
...
OUT_FILE   = Path(__file__).parent.parent / "Database" / "cocoa_currency_data.parquet"
...
# USD weight stays as constant 1.0 — no fetch needed
COCOA_WEIGHTS = {
...
def fetch_fx(start, end):
    log.info("Fetching Cocoa FX rates from ICE (%d currencies) from %s", len(FX_SYMS), start)
    frames = {}
...
def fetch_futures(start, end):
    log.info("Fetching Cocoa futures prices from ICE from %s", start)
    frames = {}
...
def compute_index(df):
    ...
    idx = sum(
        (out[c] / base[c]) * w
        for c, w in COCOA_WEIGHTS.items()
        if c in out.columns and c in base
    ) * 100
    ...
    all_cols = list(COLUMN_NAMES.values()) + list(FUTURES_SYMS.keys()) + ["Cocoa_Idx"]
...
def recompute_index(df):
    out = df.copy()
    parts = []
    for code, w in COCOA_WEIGHTS.items():
        if code == "USD":
...
def main():
    ...
    log.info("=" * 60)
    log.info("Cocoa Currency Ingest | %s", datetime.datetime.now().strftime("%Y-%m-%d %H:%M"))
    ...
    if OUT_FILE.exists() and not args.full:
        ...
        combined = recompute_index(combined)
    else:
